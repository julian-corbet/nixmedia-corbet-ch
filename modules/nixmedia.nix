...
# GStreamer app a host happens to run -- load to gain format support, and the tool that re-encodes
# an artifact you already have (`transcode`). Every TOOL that has no display mode, or a display
# mode that is not its default, has moved to nixsh; a plugin is not a tool at all and is tested a
# different way. See lib/media.nix's own header for all three placement rules, the worked examples
# that shaped `players`, and the one deliberate exception (mpv, filed in nixsh by stated use rather
# than by its default). A fourth, hardware-keyed VA-API driver group (`accel`) briefly lived here
# and was removed -- see lib/media.nix's own header for why; that selection is nixgpu's now.
#
  ...
  options.nixmedia = {
    ...
    selected = lib.mkOption {
      ...
      description = ''
        ...
        `players` test, and why `transcode` needed its own group rather than stretching one of the
        first two.
      '';
    };
    ...
    aurPackages = lib.mkOption {
      ...
      description = ''
        ...
        across all three groups, is an official-repo package on Arch) -- kept as its own list
