{
  description = "nixmedia — media consumption and format-shifting declared, class-wide: players (vlc, shortwave), the GStreamer/vlc plugin surface, transcode (handbrake), and library (easytag). Terminal-shaped tools (ffmpeg, mpv, cmus, yazi, chafa, timg, yt-dlp) moved to nixsh; not production (nixrecord), not streaming transport (nixremote, which owns sunshine/moonlight), and not hardware-keyed drivers (nixgpu, which owns VA-API)";
  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      ...
      homeManagerModules.nixmedia = ./home/nixmedia.nix;
      homeManagerModules.default = ./home/nixmedia.nix;
      homeManagerModules.install = ./home/nixmedia.nix;
